...
function validateGuess(guess = 0) {
  ...
  if (guess !== secret) {
    ...
    // throw new Error({code: 401, message:"That is not the secret number!"});
...
function findPlayerAveragePoints(players = {}, name="") {
  ...
  //if player has not played any games, have an error pushed to an array
    ...
    errors.push("Player has not played any games. No average calculated.")
  }
  //if player has played in less than 2 games, have an error pushed to an array
  else if(players[name].points.length < 2){
    errors.push("Player has not played enough games for an accurate average.")
  }
  ...
  let sum = points.reduce((acc, element)=>{
    acc += element;
    return acc;
  },0)
...
console.log(findPlayerAveragePoints(players, "William"));
